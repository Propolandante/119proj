...
function loadPins() {
	...
	for (i = 0; i < imageObjectData.objects.length; i++)
	{
		...
		objectLabel.addChild(makeObjectTags(imageObjectData.objects[i].x, imageObjectData.objects[i].y, imageObjectData.objects[i].name));
			...
			var from = new Point(objectLabel.children['pin'].position.x- objectLabel.children['text'].content.length*8, 
		  	                      objectLabel.children['pin'].position.y+8);
			var to   = new Point(objectLabel.children['pin'].position.x+objectLabel.children['text'].content.length*8, 
			                   	  objectLabel.children['pin'].position.y+30);
		...
		pin.onMouseEnter = function(event) 
		{
			...
			if(this.parent.children['text'])
			{
				...
				this.parent.children['rect'].visible = true;
			}
		...
		pin.onMouseLeave = function(event) 
		{
			...
			if(this.parent.children['text'])
			{
				...
				this.parent.children['rect'].visible = false;
			}
    ...
    // press enter to finish typing
  	zxcTextBox.onkeydown = function(event){ 
  		...
  		if (event.keyCode == '13')
  		{
  			...
  			// for relationship backdrop 
  			//	console.log ("??"+group.children[0]);
  			
  			 // var from = new Point(x-group.children['text'].content.length*8, 
		  	                      // y-10);
			 // var to   = new Point(x+group.children['text'].content.length*8, 
			                   	  // y+10);
			 // var rect = new Path.Rectangle(from, to);
